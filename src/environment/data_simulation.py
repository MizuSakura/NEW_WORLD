...
def run_simulation(env, signal_generator, signal_type='pwm', signal_params=None,
    ...
    Args:
        env : instance of RC_Tank_Env
        signal_generator : instance of SignalGenerator
        signal_type : 'pwm', 'step', 'ramp', 'impulse', 'sinusoid'
        signal_params : dict, parameters for the selected signal
        log_dt : float, log every log_dt seconds
        folder_save_csv : Path, folder to save CSV
        folder_save_plot : Path, folder to save plots
    ...
    plt.title(f"Simulation - {signal_type}")
    plt.xlabel("Time [s]")
...
if __name__ == "__main__":
    ...
    env = RC_Tank_Env(control_mode=MODE, max_action_volt=MAX_VOLT,
    ...
    # ทดลองรันหลาย duty cycle ของ PWM
    for duty in np.linspace(0.1, 0.9, 5):
        run_simulation(env, sg, signal_type='pwm',
                       signal_params={'amplitude':1, 'freq':0.5, 'duty':duty},
                       log_dt=DT_LOG, folder_save_csv=folder_csv,
                       folder_save_plot=folder_plot,
                       file_prefix=f"pwm_duty_{duty:.2f}")
